...
const {addWalletTransactionToDb} = require('./userController');
const {initKhaltiPayment, verifyKhaltiPayment} = require("../helper/khalti");
const {WEBSITE_NAME} = require("../env");
const {getOrderPlacedPage} = require("../services/render");
        ...
        if (addressId && paymentMethod) {
            ...
            newOrder.save()
                .then(async (savedOrder) => {
                    ...
                    } else if (savedOrder.paymentMethod === 'KHALTI') {
                        await initKhaltiPayment(req, res, savedOrder, (err) => {
...
const onPaymentVerified = async (req, res, details) => {
    exports.changePaymentStatus(details.purchase_order_id, details, "PAID")
        .then(async () => {
            await Cartdb.findOneAndDelete({customerId: req.session.user._id});
            res.status(200).redirect('/user/view-order/' + details.purchase_order_id);
        })
        .catch(err => {
            console.log(err);
            res.status(500).render('error', {
                message: "Payment verified but unable to update payment status",
                errStatus: 500
            });
        });
};

const onPaymentFailed = (res) => {
    res.status(500).render('error', {
...
exports.verifyPayment = (req, res) => {
    try {
        verifyKhaltiPayment(req, res, onPaymentVerified, onPaymentFailed);
    } catch (err) {
        onPaymentFailed(res);
    }
};
                    ...
                    if (order !== null) {
                        ...
                        if (order.paymentStatus === "PAID") {
                            ...
                            await addWalletTransactionToDb(req.session.user._id, amount, "C", remarks)
                                .then((data) => {
                                    console.log(`Refund of amount "Rs.${data.amount}" is successful!`);
                ...
                .then(async (order) => {
                    if (order !== null) {
                        ...
                        if (result.modifiedCount !== order.products.length) {
                            ...
                            console.log("Re-stocked all the ordered products!");
                        }
...
exports.generateInvoicePDF = (req, res, next) => {
    ...
    const data = {
        ...
        images: {
            logo: fs.readFileSync(path.join(__dirname, "../../public/logo.png"), 'base64'),
        },
        "sender": {
            "company": WEBSITE_NAME,
            "city": "Biratnagar",
            "country": "Nepal",
            "email": "bibekdhakal8366@gmail.com"
        },
        "information": {
            "number": order[0]?._id,
        ...
        "products": products,
        "bottomNotice": WEBSITE_NAME + " " + new Date().getFullYear() + " All rights reserved.",
        "settings": {
            "currency": "NPR",
        },
    };
